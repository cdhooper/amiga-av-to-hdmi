...
The board routes the Raspberry Pi HDMI signal through an onboard HDMI
switch, and the switched output is connected to the rear-facing HDMI
connector. The onboard HDMI switch has two inputs:
...
Normal operation selects the Pi input. The secondary input can be selected
through the STM32 or by headers on the Amiga AV to HDMI board.

The secondary HDMI input is intended for a second internal HDMI source,
such as the RTG video output of a Z3660. Optionally connect video from
that source after installing in your Amiga.

On Rev7.2 the secondary HDMI input uses a female HDMI connector.
Earlier revisions use a male connector for the secondary HDMI input.

**NOTE FOR TESTING:**
You can install J5 to select video from the secondary internal HDMI connector.
If you have installed the FlashFloppy OSD firmware, you can also toggle
between the two displays by a quick press of the "DFU" button. If you have
attached the Amiga keyboard inputs to the KBDAT and KBCLK header, you can
also select between the displays using Ctrl-Amiga-1 and Ctrl-Amiga-2
on the keyboard. Finally, the Ratte Switch header on the top of the
Amiga AV to HDMI board may be connected to a Ratte Auto Monitor Switch
CIA adapter.
...
The board can also switch to the secondary internal HDMI source when that
source is connected to this board.
...
If you are following the standard FlashFloppy OSD guide, do not add an
external FlashFloppy OSD adapter when using the integrated STM32
interface. The STM32 on the Amiga AV to HDMI adapter provides this
functionality.

## 9. Connect the Amiga keyboard

If keyboard control is enabled, the STM32 interfaces with the Amiga keyboard
and provides the corresponding FlashFloppy control.
